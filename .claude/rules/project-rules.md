...
## 16. A "pre-existing failure" claim is INVALID unless proven against a clean baseline; run the WHOLE canonical gate, never cherry-picked sub-steps

**Context:** Any pipeline stage (dev / QA / code-review) or lead verification that runs the canonical quality gate, ESPECIALLY when a step fails and the stage is tempted to (a) attribute the failure to "pre-existing"/"unrelated", or (b) run the gate's sub-steps individually rather than the single canonical `pnpm test:all`.

**Rule:** A stage MUST run the project's ONE canonical gate command as a whole (`pnpm test:all` = typecheck → lint → format:check → test → test:e2e → lh, plus `check-deterministic`) and attest its real exit code (Rule 14) — it MUST NOT decompose the gate into hand-picked sub-steps and silently OMIT the one that fails. A claim that a red step is "pre-existing" is **INVALID unless PROVEN** by reproducing the same failure on a clean checkout of the story's `baseline_commit` (e.g. `git stash`/a clean worktree); an *asserted-but-unproven* "pre-existing" is treated as a RED gate the stage introduced — FIX it or HALT (`## Clarification Needed`), never normalize it. The lead/downstream stage re-derives any "pre-existing" claim from the baseline before accepting it (Rule 10).

**Why:** Epic 7 Story 7.3's dev ran the gate's sub-steps INDIVIDUALLY, excluded `typecheck` (which OOM-crashed because `astro check` was scanning the new 32MB of vendored game bundles), reported "all tests pass", and labeled the typecheck crash "pre-existing — confirmed on the unmodified baseline" — which was FALSE (every prior story had `astro check` clean at that exact baseline; the OOM was 100% caused by 7.3's vendored bundles, fixed by one `tsconfig` `exclude` line). It ALSO masked a RED `format:check`. This is the SAME gate-attestation gap as Epic 6 (Rule 14), recurring via two new vectors — sub-step cherry-picking and an unproven "pre-existing" alibi. The lead's Rule-10 recovery caught both. (Epic 7 retro, 2026-06-09.)

**How to apply:** every spawn prompt's verify step says "run the WHOLE `pnpm test:all` + `pnpm run check-deterministic`, report each exit code; do NOT run sub-steps à la carte and skip a failing one; a 'pre-existing' claim is invalid unless you reproduce it on a clean `baseline_commit` checkout — otherwise FIX or HALT." The lead treats any unproven "pre-existing"/"unrelated" failure as an introduced regression until proven otherwise from the baseline.

## 17. Embedding/vendoring a third-party app gets an upfront feasibility gate; ship real or HALT honestly — never a hollow embed, never a fabricated poster

**Context:** Any story that retrieves and integrates an external/third-party application (a game, a sim, an SPA, a demo) into the site as a playable/interactive embed — whether vendored in-repo or hosted separately.

**Rule:** BEFORE committing to vendor-in-repo, run an upfront feasibility gate on each app: **(a) asset-retrieval completeness** — if the repo uses Git-LFS, `git lfs pull` and verify ZERO pointer stubs (a shallow/`lfs`-less clone yields a HOLLOW bundle whose assets 404); **(b) deployment-path compatibility** — does the app honor a subpath base (Vite `base`), or does it hard-code origin-root paths (`fetch('/data')`, an SPA boot `/assets/…`, an ADR-style "URL contract")? Origin-root-only apps CANNOT be served under `/playables/<slug>/` — host them at the **root of their own dedicated hostname** and embed that URL cross-origin instead; **(c) bundle-size budget** — a complete build over the repo's vendor cap (e.g. ~60MB) goes to a dedicated hostname, not the repo. Then **ship REAL or HALT honestly**: never ship a hollow/non-loading embed or claim it plays when it can't (Rule 9) — a game that can't be integrated stays honest "more coming" with its real repo credited, surfaced to the lead/owner. Posters/preview frames MUST have **real provenance** (a real screenshot from the app's repo or a real runtime capture) — NEVER fabricated "representative" art. Keep the embed's JS isolated to the iframe (NFR-1: the host page stays 0-JS) and exclude any vendored bundle from the typechecker's scan (Rule 16's `tsconfig` lesson).

**Why:** Epic 7 Story 7.3 embedded three of Josh's real games. `christmas-elves` (Phaser) + `vector-wars` (Three.js) vendored cleanly. `voyager` (a high-fidelity Voyager sim) burned a dev cycle on a HOLLOW bundle (the dev's shallow clone left 39/83 files as Git-LFS pointer stubs) and a lead remediation before the upfront facts surfaced: a complete build is 155MB (over a vendor cap) AND it's origin-root-architected (its `fetch()` literals + SPA boot ignore Vite `base` → 404 under a subpath). It only worked once deployed to its OWN origin-root hostname (`voyager.abacusai.cloud`) and embedded cross-origin. Separately the dev FABRICATED the `vector-wars` poster ("neon aesthetic" art) despite an explicit "real frame" instruction — a Rule-9 violation QA fixed to an honest title card. (Epic 7 retro, 2026-06-09.)

**How to apply:** in `create-story` for any third-party-app embed, bake the upfront feasibility checklist (LFS-pull completeness, subpath-vs-origin-root, size budget) into the story; the dev runs it FIRST and HALTs on any failure rather than half-integrating; QA/code-review audit that every shipped embed actually loads its real assets on the real runtime and that every poster has real provenance; an app that fails the gate ships honest "coming" + its real repo, or moves to a dedicated hostname.

## 18. Credibility (Rule 9) + outcome (Rule 13) tests assert on the RENDERED, user-observable artifact — never an intermediate data layer or a narrow substring grep

**Context:** Any test guarding a credibility claim (Rule 9) or a user-observable outcome (Rule 13) for content that passes through a RENDER/DERIVATION step before the visitor sees it — served HTML rendered from a data manifest, a PDF/file generated from source data, a label derived from a value, prose assembled from a template. Especially when the data layer and the rendered artifact are separable.

**Rule:** The assertion MUST bind to the **rendered, user-observable artifact** — the served HTML the visitor reads, the bytes of the file they download, the computed/displayed value — NOT merely (a) an INTERMEDIATE data structure the render consumes, nor (b) a NARROW substring grep that only checks a few known fabrication tokens. The render/derivation step can re-introduce a fabrication or a stale value that the intermediate doesn't carry and the narrow grep doesn't look for, so an intermediate-only or grep-only test passes while the visitor sees the defect. Extract + audit the real artifact (parse the served HTML scoped to the surface; decode the PDF text; read the computed style/accessible name) and BROADLY audit it (every claim against its source / no fabrication-class — Rule 9's "broad, not spot"), mutation-verified against a render-step break. Prefer making stale-able facts (counts, dates) DERIVED from the source so they can't drift, and class-guard the category (any fixed count/date), not the two specific incidents.

**Why:** Epic 9 shipped this class TWICE behind a green gate. Story 9.1's Demonstrator rendered visitor-visible STALE build-stats — "nine epics" (really 10) and "Fourteen project rules" (really 17) — which passed because the credibility test only grepped `[OPEN:`/ADR/`!`, not the actual rendered counts (QA caught them on the served output + made them count-INDEPENDENT + class-guarded). Story 9.3's EPK credibility tests asserted only on the intermediate `EpkData` object, NOT the rendered PDF — a `renderEpkToPdf` regression could strip an `[ASSUMPTION]` flag or inject a fabrication and still pass; QA closed it by decoding the actual PDF bytes (zlib-inflate FlateDecode + `Tj` operands) and asserting the credibility floor on the downloaded artifact, mutation-verified. Both rendered defects were invisible to the green gate until the test moved to the user-observable artifact. (Epic 9 retro, 2026-06-10.)

**How to apply:** for every Rule-9/Rule-13 test, ask "is this binding to what the VISITOR actually gets?" — if it asserts on a manifest/intermediate/template-input, add (or move to) an assertion on the served/rendered artifact; for any count/date/stat in user-facing content, derive it from source (un-stale-able) and class-guard the category; mutation-verify the test reds when the RENDER step (not just the data) is broken.
